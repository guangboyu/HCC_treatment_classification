...
def train(X, y):
    ...
    plt.plot(
        ...
        tpr["micro"],
        color="blue",
        label="Micro-averaged ROC curve (area = %0.2f)" % roc_auc["micro"],
    )
...
def cross_validation_roc_multi_class(
    ...
    for name, model in models.items():
        ...
        for train_index, test_index in cv.split(X, y):
            ...
            # Manually get predict class
            # get the index of the maximum value of each row
            max_index = np.argmax(y_score, axis=1)
            # convert to one-hot format
            y_pred = np.eye(y_score.shape[1], dtype=int)[max_index]
            # Accuracy
...
if __name__ == "__main__":
    ...
    features = [
        ...
        "diagnostics_Image-original_Maximum.1",
        "original_firstorder_Entropy.1",
        "log-sigma-2-0-mm-3D_ngtdm_Contrast.1",
        "log-sigma-4-0-mm-3D_gldm_DependenceNonUniformityNormalized.1",
        "log-sigma-4-0-mm-3D_ngtdm_Busyness.1",
        "wavelet-H_glrlm_LongRunLowGrayLevelEmphasis.1",
    ]
    multi_models = {
        ...
        "glm": OneVsRestClassifier(LogisticRegression()),
        "gbd": OneVsRestClassifier(GradientBoostingClassifier()),
        "NN": OneVsRestClassifier(
            MLPClassifier(
    ...
    # train(X, y)
    cross_validation_roc_multi_class(
        models=multi_models, X=X, y=y, smote=False, StratifiedK=True
    )
